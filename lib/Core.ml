...
and pat =
  | PatIgnore
  | PatLit of literal
  | PatDecons of string * V.t list
  | PatUnpack of V.t list

and binding =
  | NonRec of V.t * expr
  | Rec of (V.t * expr) list

let mk_empty_hole () =
  let rec e = EHole r and r = ref e in
  (e, r)

let rec output_pat ppf = function
  | PatIgnore -> output_string ppf "_"
  | PatLit v -> Ast.output_lit ppf v
  | PatDecons (k, args) ->
    output_string ppf k;
    List.iter (fprintf ppf " %a" V.output) args
  | PatUnpack [] -> output_string ppf "()"
  | PatUnpack (x :: xs) ->
    fprintf ppf "(%a" V.output x;
    List.iter (fprintf ppf ", %a" V.output) xs;
    output_string ppf ")"
...
let rec collect_free : expr -> V.Set.t = function
  | ELit _ | ERaise _ -> V.Set.empty
  | EVar n -> V.Set.singleton n
  | EApp (p, q) | ESet (p, _, q) ->
  ...
  | ELam (n, e) -> V.Set.remove n (collect_free e)
  | ELet (NonRec (n, i), e) ->
...
and collect_captures : pat -> V.Set.t = function
  ...
  | PatDecons (_, xs) | PatUnpack xs ->
    List.fold_left (fun s n -> V.Set.add n s) V.Set.empty xs
...
and is_statically_constructive (names : V.Set.t) : expr -> bool = function
  ...
  | ELet (Rec vb, e) ->
    let rec loop dset = function
      ...
      | (n, i) :: xs when is_statically_constructive names i ->
