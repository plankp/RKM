  ...
  > in True & False
  > EOF
  let ((&)$1 : Bool -> Bool -> Bool) = \($0 : Bool) -> \($1 : Bool) -> match ($0 : Bool) with { False -> False; True -> match ($1 : Bool) with { False -> False; True -> True; }; } in let ((&) : Bool -> Bool -> Bool) = ((&)$1 : Bool -> Bool -> Bool) in ((&) : Bool -> Bool -> Bool) True False
  : Bool
